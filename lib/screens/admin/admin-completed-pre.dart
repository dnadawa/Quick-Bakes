...
import 'package:quickbakes/screens/admin/admin-completed.dart';
import 'package:quickbakes/screens/admin/admin-order-info.dart';
import 'package:quickbakes/widgets/custom-text.dart';

class AdminCompletedPre extends StatefulWidget {
  @override
  _AdminCompletedPreState createState() => _AdminCompletedPreState();
}

class _AdminCompletedPreState extends State<AdminCompletedPre> {
  var requestList;
  getData(){
    Firestore.instance.collection('orders').where('status', isEqualTo: 'Completed').where('withdrawn',isEqualTo: true).snapshots().listen((datasnapshot){
      setState(() {
              ...
              return GestureDetector(
                onTap: (){
                  Navigator.push(context, CupertinoPageRoute(builder: (context){
                    return AdminCompleted(id: orderID,userEmail: userEmail,bakeryEmail: bakeryEmail,bakeryName: bakeryName,price: price,);}));
                },
                child: Padding(
